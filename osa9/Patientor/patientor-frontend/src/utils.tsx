...
const parseToString = (param: any, paramName: string): string => {
  if (!param || !isString(param)) {
    throw new InvalidPatientError(
      `Incorrect or missing ${paramName}: ${String(param)}`
    );
  }
  return param;
};
...
const parseGender = (gender: any): Gender => {
  if (!gender || !isString(gender) || !isGender(gender.toLowerCase())) {
    throw new InvalidPatientError(`Incorrect or missing gender: ${String(gender)}`);
  }
...
const parseDateOfBirth = (dateOfBirth: any): string => {
  if (!dateOfBirth || !isString(dateOfBirth) || !isDate(dateOfBirth)) {
    throw new InvalidPatientError(
      `Incorrect or missing date of birth: ${String(dateOfBirth)}`
    );
  }
  return dateOfBirth;
};
